...
# def main():
    # init = strxor(msgs[0].decode('hex'), msgs[1].decode('hex'))
    # for i in range(2, 10):
    #     init = strxor(init, msgs[i].decode('hex'))
    # return init

if __name__ == '__main__':
    # print main()
    keys = []
    init = strxor(msgs[0], msgs[1])
    print init
    ...
    #     arr2 = bytearray(key)
    #     print init
    #     if init[i] >= 'a' || init[i] <= 'z' || init[i] >= 'A' || init[i] <= 'Z':
    #         arr1[i] = ord(init[i]) ^ ord(' ') ^ ord(msg[0])
    #         arr2[i] = 
